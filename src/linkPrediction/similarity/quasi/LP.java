...
/*	public Map<Pair<Number>, Double> run() {
			...
			for(Number nodeId1:allNodeIdSet){
				...
				for(Number nodeId2:allNodeIdSet){
					...
					if(!nodeId2.equals(nodeId1) && !adjNodeIdSet.contains(nodeId2)){
						...
						temp = this.commonAdjNodeId(net2,nodeId1, nodeId2).size()+this.pathCountByTriple(net2, nodeId1, nodeId2).size()*this.p;
						score.put(p, temp);
					}
				}
			}
	 ...
	 *  @return Set<Number>
	 */
	private Set<Number> pathCountByTriple(Network net2, Number preNodeId, Number postNodeId){
