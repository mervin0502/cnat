package linkPrediction.similarity.local;
...
public class AA extends Similarity {

	/**
	 * 构造函数
	 * @param net1 网络
	 */
	public AA(Network net1) {
		super(net1);
	 ...
	 * @throws E 异常输出
	 */
	public AA(Network net1, int k) throws E {
		super(net1, k);
		// TODO Auto-generated constructor stub
	}
	@Override
/*	public Map<Pair<Number>, Double> run() {
		...
		for(int i = 0; i < size; i++){
			...
			for(Number nodeId:commonAdj){
				score += (double)1/Math.log10(net2.getAdjNodeId(nodeId).size());
			}
			map.put(p, score);
		}
		return map;
				...
				for(Number nodeId2:allNodeIdSet){
					...
					if(!nodeId2.equals(nodeId1) && !adjNodeIdSet.contains(nodeId2)){
						...
						for(Number nodeId3:commonAdj){
							temp += (double)1/Math.log10(net2.getAdjNodeId(nodeId3).size());
						}
						score.put(p, temp);
					}
				}
