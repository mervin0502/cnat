...
import feature.interfaces.InterfaceBetweenness;
...
//		HashMap<Number, Double> betweenness = new HashMap<Number, Double>();
	...
	******************************************************************************************
	...
	private int accumulateChildNodes(Network net, Number preNodeId, HashMap<Number, Integer> nodeNum,HashMap<Number, LinkedList<Number>> parent){
		//Set<Number> nodeSet = net.topology.keySet();//所有的节点集合
		//HashMap<Number, Integer> nodesBetweenness = new HashMap<Number, Integer>();
		
		//HashMap<Number, Integer> nodeNum = new HashMap<Number, Integer>();
		
		Number adjNodeId = null;
		int num = 0;
		//获取邻接点
		HashSet<Number> adjNodeSet = net.getAdjNodeId(preNodeId);
	 ...
	 * 获取边介数
	 */
	private double getEdgeBetweennessByRecursion(Network net, Number preNodeId, Number rootId, PairMap<Number, Double> edgeBetweenness, HashMap<Number, LinkedList<Number>> parent, HashMap<Number, Integer> visit, HashSet<Pair> s){
		Number adjNodeId = null;
...
/*		if(preNodeId.equals(rootId)){
			nodeBetweenness.put(rootId, (double) 0);
		}*/
		//nodeBetweenness.put(adjNodeId, temp1);
		//nodeBetweenness.put(preNodeId, value)
		//返回，以便于父节点统计
		return temp1;
	}
	@SuppressWarnings("unchecked")
	private Map<String, Object> betweenArgs(){
		...
		for (Iterator<Number> iterator = nodeSet.iterator(); iterator.hasNext();) {
			...
			HashSet<Number> s = new HashSet<Number>();
			s.add(nodeId);
			//nodeBetweenness.put(nodeId, 0.0);
			getNodeBetweennessByRecursion((Network)netArgs.get("network"), nodeId, nodeId, nodeBetweenness, (HashMap<Number, LinkedList<Number>>)netArgs.get("parent"), (HashMap<Number, Integer>)netArgs.get("visit"),s);
			//nodeBetweenness.put(nodeId, (double) 0);
/*			visit = (HashMap<Number, Integer>)netArgs.get("visit");
			visit.remove(nodeId);
			Collection<Number> collection = visit.keySet();
			for(Iterator<Number> iterator1 = collection.iterator(); iterator1.hasNext();){
				nodeId2 = iterator1.next();
				if(nodeBetweenness.get(nodeId2) != null){
					temp = nodeBetweenness.get(nodeId2);
				}
				nodeBetweenness.put(nodeId2, temp+visit.get(nodeId2)*nodeNum.get(nodeId2));
			}*/
		}
		////D.p(nodeBetweenness.get(2));
		return args;
...
/*	public static void main(String[] args){
		Network net = new Network("../data/2014.txt", NetType.INDIRECTED);
		Betweenness b = new Betweenness(net);
		b.edgesBetweenness(net);
		//b.nodesBetweenness(net);
		Set<Number> nodes = net.topology.keySet();
		HashMap<Number, Double> nodeBetweenness = new HashMap<Number, Double>();//某个节点的介数
		for (Iterator iterator = nodes.iterator(); iterator.hasNext();) {
			Number number = (Number) iterator.next();
			b.shortestPathNet1(net, number, nodeBetweenness);
		}
		//D.p(nodeBetweenness.get(2));
		//b.edgesBetweenness(net);
		D.m();
		//b.shortestPathNet(net, 1);
		//b.betweenArgs();
		//Map<String, Object>  netArgs= null;
		//netArgs = b.shortestPathNet(net, 2);
		//HashMap<Number, Double> nodeBetweenness = new HashMap<Number, Double>();//某个节点的介数
		//HashMap<Number, Integer> visit = null;
		//b.getNodeBetweennessByRecursion((Network)netArgs.get("network"), 2, nodeBetweenness, (HashMap<Number, LinkedList<Number>>)netArgs.get("parent"), (HashMap<Number, Integer>)netArgs.get("visit"));

	}*/
	
/*	private Map<String, Object> shortestPathNet1(Network net, Number preNodeId, HashMap<Number, Double> nodeBetweenness){
		preNodeId = MathTool.str2Number(Network.getNumberType(), preNodeId.toString());
		...
		*//**
		 ...
		 *//*
		//HashSet<Number> s = new HashSet<Number>();//已遍历的节点
		Stack<Number> s = new Stack<Number>();//已遍历的节点
		
		HashMap<Number, Double> distance = new HashMap<Number, Double>();//有preNodeId->v的距离 
		HashMap<Number, Integer> visit = new HashMap<Number, Integer>();//在最短路径网中由preNodeId->v有几条路径可走 
		//HashMap<Number, Double> nodeBetweenness = new HashMap<Number, Double>();//某个节点的介数
		PairList<Number, Number> edges = new PairList<Number, Number>();//最短路径网中的边
		...
		*//**
		...
		//distance.put(preNodeId, 0);
		visit.put(preNodeId, 1);
		parent.put(preNodeId, new LinkedList<Number>());//根节点的父节点列表为空
		Number nodeId = null;
		//int curLevel = 0;//当前的层数
 		...
 		*//**************************************************************
 		...
 		while(!s.isEmpty()){
 			...
 			if(parent.containsKey(nodeId)){
 				for(it = parent.get(nodeId).iterator();it.hasNext();){
 					adjNodeId = it.next();
 					if(!adjNodeId.equals(preNodeId)){
	 					if(nodeBetweenness.containsKey(adjNodeId)){
	 						nodeBetweenness.put(adjNodeId, nodeBetweenness.get(adjNodeId)+((double)visit.get(adjNodeId)/visit.get(nodeId))*(1+(nodeBetweenness.get(nodeId)==null?0:nodeBetweenness.get(nodeId))));
	 					}else{
	 						nodeBetweenness.put(adjNodeId, ((double)visit.get(adjNodeId)/visit.get(nodeId))*(1+(nodeBetweenness.get(nodeId)==null?0:nodeBetweenness.get(nodeId))));
	 					}
	 				}
 				}
 			}
